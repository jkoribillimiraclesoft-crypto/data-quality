...
if uploaded_file:
    ...
    with tab_rules:
        ...
        if not dim_summary.empty:
            # Horizontal bar with data labels - clean, professional, reads
            # well at any width (labels sit on the bar, axis compresses
            # gracefully on narrow screens since it's using container width).
            ...
            fig_dim.update_layout(
                xaxis=dict(range=[0, 110], title="Pass Rate %"),
                yaxis_title="",
                height=max(260, 60 * len(dim_sorted)),
