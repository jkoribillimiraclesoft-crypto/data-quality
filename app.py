...
from storage import (
    ...
    get_latest_schema_by_label, list_known_datasets, DB_PATH
)
...
if uploaded_file:
    ...
    with tab_profile:
        ...
        # Compared by dataset_label (the uploaded filename), NOT dataset_id.
        # dataset_id is itself derived from the schema, so comparing "current
        # schema" to "the schema last saved under this dataset_id" can only
        # ever find an identical schema or nothing at all - it can never
        # detect an actual change. Matching on the label instead finds the
        # last upload of this same file regardless of whether its shape
        # changed, which is what makes drift detection possible.
        previous_schema, previous_dataset_id = get_latest_schema_by_label(dataset_label)
        if previous_schema:
            ...
            if diff["has_changes"]:
                ...
                st.success("No schema changes since the last upload of this file.")
            if previous_dataset_id != dataset_id:
                st.caption(
                    f"Compared against the previous version of **{dataset_label}** "
                    f"(dataset ID changed: `{previous_dataset_id}` \u2192 `{dataset_id}`)."
                )
        else:
            st.info(f"No previous upload of **{dataset_label}** found - this is the first run for this file.")
