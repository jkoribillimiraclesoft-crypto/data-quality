...
if uploaded_file:
    ...
    with tab_overview:
        ...
        health_df = profile["column_health"].sort_values("health_score")
        fig = go.Figure()
        for _, row in health_df.iterrows():
            color = FAIL_RED if row["health_score"] < 60 else (WARN_AMBER if row["health_score"] < 85 else PASS_GREEN)
            fig.add_shape(type="line", x0=0, x1=row["health_score"], y0=row["column"], y1=row["column"],
                          line=dict(color="#D1D5DB", width=2))
        fig.add_trace(go.Scatter(
            x=health_df["health_score"], y=health_df["column"], mode="markers",
            marker=dict(size=14, color=health_df["health_score"], colorscale=HEALTH_SCALE, cmin=0, cmax=100,
                        line=dict(width=1, color="white")),
            text=health_df["health_score"], hovertemplate="%{y}: %{x}<extra></extra>",
        ))
        fig.update_layout(xaxis=dict(range=[0, 100], title="Health Score"), yaxis_title="",
                          showlegend=False, height=max(280, 32 * len(health_df)))
        st.plotly_chart(fig, use_container_width=True)

        c1, c2 = st.columns(2)
        with c1:
            st.subheader("Missing Value Heatmap")
            sample_df = df.sample(min(200, len(df)), random_state=1).reset_index(drop=True)
            ...
            fig_null = go.Figure(data=go.Heatmap(
                ...
                colorscale=[[0, "#EEF2F6"], [1, BRAND_BLUE]],
                showscale=False,
                hovertemplate="Column: %{y}<br>Row: %{x}<br>Missing: %{z}<extra></extra>",
            ))
            fig_null.update_layout(xaxis_title=f"Row sample (n={len(sample_df)})", yaxis_title="",
                                   height=max(280, 28 * len(null_matrix.columns)))
            st.plotly_chart(fig_null, use_container_width=True)
            st.caption("Blue = missing value. Sampled rows shown for readability on large datasets.")
        with c2:
            ...
            if numeric_cols:
                ...
                for i, col in enumerate(numeric_cols):
                    fig_out.add_trace(go.Box(y=df[col], name=col, marker_color=BRAND_BLUE,
                                             boxpoints="outliers", marker=dict(color=FAIL_RED, size=4)))
                fig_out.update_layout(showlegend=False, yaxis_title="Value",
                                      height=max(280, 28 * len(null_matrix.columns)))
                st.plotly_chart(fig_out, use_container_width=True)
                st.caption("Red points beyond the whiskers are statistical outliers (IQR method).")
            else:
        ...
        with dcol1:
            ...
            st.metric("Duplicate %", f"{dup['duplicate_pct']}%")
        with dcol2:
            unique_rows = summary["row_count"] - dup["duplicate_rows"]
            fig_dup = go.Figure(data=[go.Pie(
                labels=["Unique Rows", "Duplicate Rows"],
                values=[unique_rows, dup["duplicate_rows"]],
                hole=0.6,
                marker=dict(colors=[BRAND_BLUE, FAIL_RED]),
            )])
            fig_dup.update_layout(height=220, margin=dict(t=10, b=10, l=10, r=10),
    ...
    with tab_rules:
        ...
        if not dim_summary.empty:
            ...
            fig_dim.add_trace(go.Scatterpolar(
                r=dim_summary["pass_rate_pct"].tolist() + [dim_summary["pass_rate_pct"].iloc[0]],
                theta=dim_summary["dimension"].tolist() + [dim_summary["dimension"].iloc[0]],
                fill="toself", fillcolor="rgba(0,171,232,0.25)",
                line=dict(color=BRAND_BLUE, width=2), marker=dict(size=6),
            ))
